...
export function Schedule() {
  ...
  const isCurrentClass = (item: typeof SCHEDULE[0]) => {
  ...
  // Preview rows (blurred behind AuthGate for guests)
  const PreviewRows = () => (
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <section className="relative py-20 bg-gradient-to-b from-gray-900 to-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <Link
              to="/classes"
              className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500 to-red-600 px-6 py-3 rounded-full font-semibold hover:shadow-lg hover:shadow-orange-500/40 transition-all group"
            >
      ...
      {/* Schedule content — gated for guests */}
      <AuthGate
        title="View the Full Schedule"
        description="Log in to browse all classes, filter by day or type, and see your booked sessions at a glance."
        preview={<PreviewRows />}
      >
        {/* Info banner — logged in users */}
        {isLoggedIn && bookings.length > 0 && (
          <div className="bg-green-500/5 border-b border-green-500/15">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4 flex-wrap">
              ...
              <Link
                to="/"
                className="text-xs text-green-500 hover:text-green-300 transition-colors flex items-center gap-1 shrink-0"
              >
        ...
        <section className="py-12" ref={scheduleTopRef}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <AnimatePresence mode="wait">
              <motion.div
                key={`${currentPage}-${selectedDay}-${selectedType}-${selectedTime}`}
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2 flex-wrap">
                            ...
                            {slotBooked && (
                              <span className="inline-flex items-center gap-1 bg-green-600/20 text-green-400 text-xs font-semibold px-2.5 py-1 rounded-full border border-green-500/30">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm text-gray-400">
                            <div className="flex items-center gap-2">
                              <Calendar className="w-4 h-4 text-orange-500" />{item.day}
                            </div>
                            <div className="flex items-center gap-2">
                              <Clock className="w-4 h-4 text-orange-500" />{item.time} ({item.duration} min)
                            </div>
                            <div className="flex items-center gap-2">
                              <MapPin className="w-4 h-4 text-orange-500" />{item.room}
                            </div>
                            <div>
                              <span className="font-medium text-gray-300">Instructor:</span> {item.instructor}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-5 shrink-0">
                          {/* Spots left */}
                          <div className="text-center">
                            <p className={`text-2xl font-bold ${getSpotsLeft(item.id) <= 5 ? "text-red-400" : "text-orange-500"}`}>
                              {getSpotsLeft(item.id)}
                            </p>
                            <p className="text-xs text-gray-400">Spots Left</p>
                          </div>
                        </div>
            ...
            {filteredSchedule.length === 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
                ...
                <button
                  onClick={() => { setSelectedDay("All"); setSelectedType("All"); setSelectedTime("All"); }}
                  className="bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/30 text-orange-400 px-6 py-2.5 rounded-lg font-medium transition-all"
                >
                  Clear Filters
                </button>
              </motion.div>
            ...
            {totalPages > 1 && (
              <div className="mt-10 flex flex-col sm:flex-row items-center justify-between gap-4">
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 rounded-lg border border-orange-500/30 text-sm font-medium transition-all disabled:opacity-30 disabled:cursor-not-allowed enabled:hover:bg-orange-500/10 enabled:hover:border-orange-500/60 enabled:hover:text-orange-400"
                >
                      ...
                      <button
                        ...
                        className={`w-9 h-9 rounded-lg text-sm font-medium transition-all ${
                          currentPage === page
                            ...
                            : "border border-orange-500/20 text-gray-400 hover:border-orange-500/50 hover:text-orange-400 hover:bg-orange-500/5"
                        }`}
                      >
                        {page}
                      </button>
