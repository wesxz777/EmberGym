...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        // 🔥 THE FIX: Knock on the API Door!
        const response = await api.post("/api/login", payload);

        if (response.status === 200) {
          
          // Grab the user AND the strict token generated by AuthController
        ...
        // Now React will properly catch the 401 Unauthorized from Laravel!
  ...
  const handleSendReset = async (e: React.FormEvent) => {
    ...
    try {
      await api.post("/api/forgot-password", { email: forgotEmail.trim() });
